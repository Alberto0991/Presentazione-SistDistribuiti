...
\item Sia $n_i$ il numero di canditati che raggiungono stage$=i$;
...
\vskip2em
\begin{itemize}
\item Se un candidato conquista un territorio di dimensione $n/2 + 1$ allora nessun altro candidato può diventare \textit{leader};
\item Un candidato che raggiunge stage$=n/2 + 1$ diventa quindi \textit{leader} e potrà inviare un messaggio di terminazione in broadcast a tutti gli altri nodi.
\end{itemize}
\end{frame}
...
\vskip0.5em
$n-1+\sum_{i=1}^{n/2} 4n_i \leq n-1+4n\sum_{i=1}^{n/2} 1/i = 4nH_{n/2}+n-1 $

\vskip1.5em
Ricordando che $log(1+n)\leq H_n \leq 1+logn$ si ha che:

\vskip0.5em
$M[CompleteElect] \leq 4n(1+logn/2)+n-1 = 4n+4nlogn-4nlog2+n-1 = 4nlogn+2.22n-1$
...
Combinando in maniera appropriata la strategia appena descritta con quella Na{\"i}ve si può costruire un protocollo che offre:
\vskip2em
